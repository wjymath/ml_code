...
if __name__ == 'main':
    ...
    def load_data(data_path, label_path):
        with open(label_path, 'rb') as file:
            ...
            if magic != 2049:
                raise ValueError('magic number mismatch, expect 2049, got %d' % magic)
            labels = array("B", file.read())
        with open(data_path, 'rb') as fileL
            magic, size, rows, cols = struct.unpack(">IIII", file.read(16))
            if magic != 2051:
                raise ValueError('magic number mismatch, expect 2051, got %d' % magic)
    ...
    valid_feature = valid_feature_raw.reshape(1000, 1, 28, 28)
